package com.nellex.pageRank.hadoop;
...
import com.nellex.pageRank.hadoop.job1.xmlhakker.WikiLinksReducer;
import com.nellex.pageRank.hadoop.job1.xmlhakker.WikiPageLinksMapper;
import com.nellex.pageRank.hadoop.job1.xmlhakker.XmlInputFormat;
import com.nellex.pageRank.hadoop.job2.calculate.RankCalculateMapper;
import com.nellex.pageRank.hadoop.job2.calculate.RankCalculateReduce;
import com.nellex.pageRank.hadoop.job3.result.RankingMapper;

public class WikiPageRanking extends ConfigUtil implements Tool {
	...
	public int run(String[] args) throws Exception {
		boolean isCompleted = runXmlParsing(args[1], args[2]);// runXmlParsing("enwiki-sample.xml",
																// "wiki/ranking/iter00");
		if (!isCompleted)
			return 1;
		...
		for (int runs = 0; runs < 5; runs++) {
			String inPath = "s3://nxwikibucket/wiki/ranking/iter" + nf.format(runs);
			lastResultPath = "s3://nxwikibucket/wiki/ranking/iter" + nf.format(runs + 1);
		...
		isCompleted = runRankOrdering(lastResultPath, "s3://nxwikibucket/wiki/result");
	...
	private boolean runRankCalculation(String inputPath, String outputPath) throws IOException,
			ClassNotFoundException, InterruptedException {
		Configuration conf = new Configuration();
	...
	private boolean runRankOrdering(String inputPath, String outputPath) throws IOException,
			ClassNotFoundException, InterruptedException {
		Configuration conf = new Configuration();
