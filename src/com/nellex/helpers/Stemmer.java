...
public class Stemmer implements Function<Word, Word> {
	 ...
	 * file-name ...
	 * 
	 * public static void main(String[] args) throws IOException { Stemmer s =
	 * new Stemmer(); if (args[0].equals("-file")) { Iterator<Word> it =
	 * PTBTokenizer.newPTBTokenizer(new InputStreamReader(new
	 * FileInputStream(args[1]), "utf-8")); while (it.hasNext()) { Word token =
	 * it.next(); System.out.print(s.stem(token.word())); System.out.print(' ');
	 * } } else { for (String arg : args) { System.out.print(s.stem(arg));
	...
	/**
	 * Stems <code>word</code> (which must be a <code>Word</code>, or else a
	 * ClassCastException will be thrown, and returns stemmed <code>Word</code>.
	 */
